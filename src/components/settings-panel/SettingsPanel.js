// SettingsPanel: Este componente ofrece un panel de configuración lateral que permite a los usuarios personalizar la apariencia y el comportamiento de la aplicación.
// Los usuarios pueden modificar opciones como el tema, la disposición del navbar, el modo RTL y otros ajustes visuales.
// Este panel se muestra como un `Offcanvas` y está relacionado con el contexto global de la aplicación (`useAppContext`),
// permitiendo aplicar cambios globales de configuración a través de `setConfig` y `configDispatch`.
// Incluye varios componentes como `RadioItem` para selección de opciones visuales y `SubtleBadge` para mostrar novedades.
