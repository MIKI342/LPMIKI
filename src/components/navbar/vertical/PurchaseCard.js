 ...
 * 
 * Renderiza una tarjeta promocional que invita a comprar el producto Falcon.
 * Incluye un botón para cerrar la tarjeta y un enlace al sitio de compra.
 * 
 * Ejemplo de uso:
 * <PurchaseCard />
 */
const PurchaseCard = () => {
  const [show, setShow] = useState(true); // Controla la visibilidad de la tarjeta

  return (
    show && (
      <div className="settings my-3">
        <Card className="p-0 rounded-2 position-relative">
          <div className="position-absolute" style={{ right: '6px', top: '3px' }}>
            <FalconCloseButton
              size="sm"
              className="fs-11"
              noOutline
