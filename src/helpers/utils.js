/**
 * Este archivo contiene una colección de funciones de utilidad reutilizables en toda la aplicación.
 * Proporciona varias funcionalidades que facilitan el manejo de datos, manipulación de arrays, manejo de estilos y
 * colores, formateo de números, cookies, y otras herramientas para componentes y configuraciones de UI.
 *
 * **Funcionalidades destacadas**:
 * - **Manejo de Almacenamiento y Cookies**:
 *   - `getItemFromStore` y `setItemToStore` para interactuar con el almacenamiento local (localStorage).
 *   - `getCookieValue` y `createCookie` para crear y leer cookies.
 * - **Formatos de Fechas y Duración**: Utiliza `dayjs` para manejar el formateo de fechas y cálculos de duración.
 * - **Formateo Numérico**: `numberFormatter` para formatear números en formatos legibles (K, M, B).
 * - **Manipulación de Colores**: Conversión de colores en diferentes formatos (hex a RGB, rgba).
 * - **Funciones de Paginación**: Herramientas para dividir arrays, como `getPaginationArray`, `pagination`, y `chunk`.
 * - **E-Commerce Utilidades**: Funciones de cálculo de precios y descuentos para aplicaciones de comercio electrónico.
 * - **Manejo de Rutas**: Herramientas para manipular rutas y estructuras de navegación (`flatRoutes`, `getFlatRoutes`).
 * - **Generación de Slug**: Convierte texto a un formato de slug amigable para URL (`slugifyText`).
 *
 * Estas utilidades permiten mantener un código más limpio y centralizar tareas comunes, facilitando su mantenimiento.
 */
